...
async function init() {
  if (STATIC_MODE) $("staticBanner").hidden = false;
  const health = await fetch(STATIC_MODE ? "data/health.json" : "/api/health").then((r) => r.json());
  ...
  $("liveBadge").textContent = STATIC_MODE ? "Recorded replay demo (static build, no server)" : health.live ? "Token Factory connected" : "Replay-only mode (no API key on this server)";
  $("runLive").disabled = !health.live;
  state.samples = await fetch(STATIC_MODE ? "data/samples.json" : "/api/samples").then((r) => r.json());
...
async function streamStatic(sampleId) {
  const text = await fetch(`data/samples/${encodeURIComponent(sampleId)}/run.ndjson`).then((r) => {
  ...
  for (const raw of text.split("\n")) {
    ...
    const t = Number(event.t ?? previous);
    const delay = Math.max(0, t - previous) * 1000;
    previous = t;
    if (delay) await new Promise((r) => setTimeout(r, Math.min(delay, 8000)));
    event.replay = true;
    handle(event);
  }
}
...
async function run(mode) {
  ...
  try {
    if (STATIC_MODE) {
      await streamStatic(state.sampleId);
    } else {
